...
void delta_region_class(float *output, float *delta, int index, int cur_class, int classes, tree *hier, float scale, float *avg_cat)
{
    int i, n;
    if(hier){
        float pred = 1;
        while(cur_class >= 0){
            pred *= output[index + cur_class];
            int g = hier->group[cur_class];
            int offset = hier->group_offset[g];
            for(i = 0; i < hier->group_size[g]; ++i){
                delta[index + offset + i] = scale * (0 - output[index + offset + i]);
            }
            delta[index + cur_class] = scale * (1 - output[index + cur_class]);
        ...
        *avg_cat += pred;
        ...
        for(n = 0; n < classes; ++n){
            delta[index + n] = scale * (((n == cur_class)?1 : 0) - output[index + n]);
            if(n == cur_class) *avg_cat += output[index + n];
        }
    }
}
    ...
    for (b = 0; b < l.batch; ++b) {
        if(l.softmax_tree){
            ...
            for(t = 0; t < 30; ++t){
                ...
                if(!truth.x) break;
                int cur_class = state.truth[t*5 + b*l.truths + 4];
                float maxp = 0;
                int maxi = 0;
                if(truth.x > 100000 && truth.y > 100000){
                    for(n = 0; n < l.n*l.w*l.h; ++n){
                        ...
                        float p = scale*get_hierarchy_probability(l.output + index, l.softmax_tree, cur_class);
                        if(p > maxp){
                            maxp = p;
                            maxi = n;
                        }
                    ...
                    delta_region_class(l.output, l.delta, index, cur_class, l.classes, l.softmax_tree, l.class_scale, &avg_cat);
        ...
        for(t = 0; t < 30; ++t){
            ...
            delta_region_class(l.output, l.delta, best_index + 5, cur_class, l.classes, l.softmax_tree, l.class_scale, &avg_cat);
            ++count;
            ++class_count;
        }
    }
